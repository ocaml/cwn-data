...
[[https://alan.petitepomme.net/cwn/2026.06.23.html][Previous Week]] [[https://alan.petitepomme.net/cwn/index.html][Up]] [[https://alan.petitepomme.net/cwn/2026.07.07.html][Next Week]]
...
We are pleased to announce a new release of [[https://gitlab.inria.fr/fpottier/sek][Sek]], a library that offers efficient ephemeral (mutable) and persistent (immutable) /sequence/ data structures, plus efficient conversions between these two forms.

This new release introduces optimizations that speed up several operations, including random access (~get~, ~set~), splitting, and concatenation. For details, please see the [[https://gitlab.inria.fr/fpottier/sek/-/blob/master/CHANGES.md?ref_type=heads][change log]].
...
Let me add this link to [[https://cambium.inria.fr/~fpottier/sek/doc/sek/Sek/][the documentation]].
...
- An *alternative effects backend* based on the [[https://github.com/WebAssembly/stack-switching][Stack Switching proposal]].
...
A few changes are source-incompatible — mostly to fix incorrect bindings and behavior — and may require updating existing code. See the [[https://github.com/ocsigen/js_of_ocaml/wiki/Upgrade-from-6.3.0-to-6.4.0][migration guide]].

See the [[https://ocsigen.org/js_of_ocaml/][documentation]] and the [[https://github.com/ocsigen/js_of_ocaml/blob/master/CHANGES.md][full changelog]].
...
1. *If you use OCaml:* I've patched the compiler to use LRgrep. Please try it via opam:
...
2. *If you use Menhir:* Give LRgrep a try on your own grammars if you want to customize  error messages.
   ...
   * [[https://github.com/fpottier/lrgrep-example][General Examples]]
   * [[https://github.com/let-def/lrgrep/blob/main/examples/ocaml/parser/errors.lrgrep][OCaml Error Specifications]]

Integration with Merlin is also coming soon ([[https://github.com/ocaml/merlin/pull/2072][PR #2072]]).
...
v2.1.1 adds ~duras build~, which converts a note collection to a static HTML website. 7,300 notes in \~220ms, single-threaded, no caching. Notes use [[https://codeberg.org/duras/sigline][sigline]] format — a plain-text structured notation where the leading character of each line declares its type.
...
[[https://www.lexifi.com/][LexiFi]] is hiring! We are looking for a full-time Software Engineer to join our core development team.
...
I'd like to share a small project I've been working on: *owebview*, a set of OCaml bindings for [[https://github.com/webview/webview][webview]].

*** What is webview?

[[https://github.com/webview/webview][webview]] is a tiny, cross-platform library for building desktop GUIs using the operating system's built-in web engine — WebKit on macOS, WebKitGTK on Linux, and WebView2 on Windows. Instead of shipping a whole browser like Electron, you reuse the system one, so your apps stay small. You create a window, point it at some HTML (or a URL), and you can call back and forth between the page's JavaScript and your host language.
